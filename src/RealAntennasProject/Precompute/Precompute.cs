...
namespace RealAntennas.Precompute
{
    ...
    internal class Precompute
    {
        ...
        ///
        /// For UNLOADED vessels: Fall back to the stock CommNet signal-strength modifier
        /// </summary>
        private static float GetPlasmaSeverity(RACommNode node, out float bestFreqHz)
        {
            ...
            else
            {
                ...
                // Fall back to the stock CommNet signal-strength modifier, which
                // KSP derives from the vessel's protoVessel plasma state.
                double stockModifier = raCNV.GetSignalStrengthModifier(node);
                severity = Physics.PlasmaModifierToSeverity(stockModifier);
            }
